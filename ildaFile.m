...
+ (instancetype)sharedInstance {
    static CoordinateData *sharedInstance = nil;
...
- (void)executeCommand:(struct coordinateData *)data {
    ...
    float z = (*data).z / ILDA_AxisMax;
    //CGFloat normalizedX = self.x / 32768.0;
    CGFloat normalizedX = (CGFloat) x;
    //CGFloat normalizedY = self.y / 32768.0;
    CGFloat normalizedY = (CGFloat) y;
    //CGFloat normalizedZ = self.z / 32768.0;
    CGFloat normalizedZ = (CGFloat) z;


    if((*data).status.blanking == 0){
            float redzz = (*data).r / ILDA_Colour_Max;
            float greenzz = (*data).g / ILDA_Colour_Max;
            float bluezz = (*data).b / ILDA_Colour_Max;
            CGFloat redz = (CGFloat) redzz;
            CGFloat greenz = (CGFloat) greenzz;
            CGFloat bluez = (CGFloat) bluezz;
        
        [[LaserController sharedInstance] setColourWithRed:redz green:greenz blue:bluez];
    }else{
        [[LaserController sharedInstance] setColourWithRed:0.0 green:0.0 blue:0.0];
    }
    [[LaserController sharedInstance]  moveToX:normalizedX y:normalizedY z:normalizedZ];

    //moveTo(x, y);
    ////moveToSpeedLimit(x, y, distPerS);
    //if (!self.status.blanking) {
    //    [[LaserController sharedInstance] setColourWithRed:redz green:greenz blue:bluez];
    //} else {
    //    [[LaserController sharedInstance] setColourWithRed:0.0 green:0.0 blue:0.0];
    //}
...
@implementation IldaFileParser
+ (instancetype)sharedInstance {
    static IldaFileParser *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[self alloc] init];
    });
...
- (instancetype)initWithFileURL:(NSURL *)fileURL {
    ...
    if (self) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            executeIldaFileByName([fileURL.path UTF8String], 1);
        });
    }
    return self;
}
...
- (BOOL)parseFileWithError:(NSError **)error {
    // Implement the parsing logic based on the ILDA format specification
    // Populate the 'coordinates' array with CoordinateData objects
    // Handle different ILDA record types (e.g., 3D coordinates, color palette)
    // Use the 'CoordinateData' class to manage each point's data
    return YES;
}
...
struct statusCode readStatusCode(FILE *fp){
    ...
    return status;  // Added return statement
}
//
void parse3DCoordTrueColData(FILE *fp, int numberEntries){
    for(int i = 0; i < numberEntries; i++){
        ...
        // Reading RGB values directly from file
        data.b = fgetc(fp);  // Blue component
        data.g = fgetc(fp);  // Green component
        data.r = fgetc(fp);  // Red component
